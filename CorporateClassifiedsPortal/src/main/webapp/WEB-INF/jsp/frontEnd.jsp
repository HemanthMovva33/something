<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Login</title>
 <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>
  
  <style>
  .navbar{
  	 min-height: 80px;
  }
  .footer{
position:fixed;
left:0;
bottom:0;
width:100%;
text-align:center;
}
  </style>
</head>
<body>

	 <nav class="navbar navbar-default" style="background-color: #0000CD;">
  		<div class="container-fluid">
    	<div class="py-5">
     	<a class="navbar-brand" style="color: #FDF5E6" href="viewAllOffers">
     	<span><img src="data:image/png;base64," alt="" width="50" height="45" class="d-inline-block align-text-top">
     	Corporate Classifieds</span></a>
     	
    	</div>
    	<div class="col-lg-offset-10">
    	<ul class="nav justify-content-end navbar-nav ">
      		<li class="nav-item"><a style="color: #FDF5E6" href="viewMyProfile">
      		
      		<span><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQN6qtG51_xdYVcYcP4qhCeWEB9HtKAF4ySCA&usqp=CAU" alt="" width="30" height="25" class="d-inline-block align-text-top">
     		${uname}</span>
      		</a></li>
      		
      		
      		<li class="nav-item"><a style="color: #FDF5E6" href="logout">Logout</a></li>
      		
    	</ul></div>
  		</div>
		</nav>
		
		 <br><br>
		 
 <nav class="navbar navbar-default" style="background-color:  $light;">
  		<div>
    	 <div class="col-lg-offset-3">
    	<ul class="nav justify-content-end navbar-nav ">
      		
      		<li class="nav-item"><a style="color: $blue-800" href="#">View Offers By Category
      			<form method="GET" action="viewOffersByCategory">
				
				<select style="color: $blue-800" name="Category" id="Category">
					<option value=="none" selected disabled hidden></option>
					<option value=1>Electronics</option>
					<option value=2>Vehicles</option>
					<option value=3>Properties</option>
					<option value=4>Others</option>
				</select>
			 	<input type="submit" value="Submit" style="color: #0000CD">
			 	</form>
      			</a></li>
      		<li class="nav-item"><a style="color: $blue-800" href="viewOffersByTopLikes" class="text-right">Most Popular Offers</a></li>
      		<li class="nav-item"><a style="color: $blue-800" href="viewOffersByTopLikes">Filter By Date</a>
      			<form method="GET" action="/viewOffersByDate">
      			<input type="date" pattern="yyyy-MM-dd" id="date" name="date">
      			
      			<input type="submit" value="Submit" style="color: #0000CD"></form>
      		</li>
      	
    	</ul>
  		</div>
  		</div>
		</nav>
		
		
	
		
		

	<div class="container col-lg-offset-3">
	
		<c:forEach items="${Offerlist}" var="offer">
		
			<div class="row">
			<div class="col-sm-6" >
			<div class="thumbnail" style="background-color:#FAF0E6;">
			
				<p>Offer ID: ${offer.offerId}</p>
				<div class="position-absolute top-50 start-50 translate-middle">
					<p>Filters:</p>
					<p>Title : ${offer.offerTitle}</p>
					<p>Open Date: ${offer.offerOpenDate}</p>
					<p>Close Date: ${offer.offerClosedDate}</p>
				</div>
				<div class="item-type">
					<p>Description: ${offer.offerDescription}</p>
					<c:set var="catid" scope="session" value="${offer.categoryId}"/>
					<c:if test= "${catid==1}">
					<p>Category: Electronics</p></c:if>
					<c:if test= "${catid==2}">
					<p>Category: Vehicles</p></c:if>
					<c:if test= "${catid==3}">
					<p>Category: Properties</p></c:if>
					<c:if test= "${catid==4}">
					<p>Category: Others</p></c:if>
				</div>
				<div class="del-cart">
			
					<a href="incrimentLikes/${offer.offerId}" >
					<button type="button" class="btn btn-danger">Likes</button> </a>
							<span>${offer.offerLikes}</span>
				</div>
			</div>
			</div>
			</div>
		</c:forEach>
		<div class="footer">
		 <div style="background-color: rgba(0, 0, 0, 0.2);height: 50px;">
    		© 2020 Copyright:
  		</div>
  		</div> 
  		
</body>
</html>
